    ...
    request.get(url, function (err, resp, body) {
        ...
        if(pattern.test(resp.request.uri.href)) {
            ytdl.getInfo(url, ['--youtube-skip-dash-manifest'], function(err, info) {
                if(err) return res.render('listvideo', {error: 'The link you provided either not a valid url or it is not acceptable',info:info});
                ...
                res.render('listvideo', {meta: {id: info.id, formats: formats, description: info.description}, info: info });
            })
        }
        else {
            res.render('listvideo', {error: 'The link you provided either not a valid url or it is not acceptable',info:info});
        }
    });
